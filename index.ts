...
async function promptUser() {
    const currencies = Object.keys(currencyRates) as Currency[];

    const { fromCurrency } = await inquirer.prompt<{ fromCurrency: Currency }>({
        name: "fromCurrency",
    ...
    const { toCurrency } = await inquirer.prompt<{ toCurrency: Currency }>({
        name: "toCurrency",
